...
from cogs.maincopy import server_config
...
startup_extensions = ['cogs.maincopy']
...
async def on_ready():
	...
	while True:
		if len(bot.guilds) > 1:
			await bot.change_presence(status = discord.Status.online, activity = discord.Game(name = 'with Ao', type = 1))
		else:
			await bot.change_presence(status = discord.Status.online, activity = discord.Game(name = 'with Ao', type = 1))
